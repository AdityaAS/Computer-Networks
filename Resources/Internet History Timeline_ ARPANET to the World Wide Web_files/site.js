...
function reloadScripts(event_name, targetTags){
    ...
    if(targetTags.length > 0){
        ...
        tmntag.refresh(targetTags);
    } else{
        var AdNames=[];
        for(var ad_name in adSlots) if(ad_name.search(/sense[1-9]/)<0) AdNames.push(ad_name);
        //kill_ad_frames(AdNames);
        tmntag.refresh(AdNames);
    }
}
...
function popupImageSize(url){
    ...
    var height=(browseheight < heightx) ? browseheight-50:heightx;
    var width=(browsewidth < widthx) ? browsewidth-50:widthx;
...
function loadDataCache(filename, container_id){
    $jq.get( '/data_cache/'+filename+'.html?cacheme=true', function(data) {
        $jq('#'+container_id).html( data );
    });
}